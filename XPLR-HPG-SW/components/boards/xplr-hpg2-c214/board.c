    ...
    switch (id) {
        case XPLR_PERIPHERAL_LTE_ID:
            if (!on) {
                ...
                // err[2] = gpio_set_level(BOARD_IO_LTE_nRST, 0);
    ...
    switch (gpio_id) {
        ...
        case BOARD_GPIO_CONFIG_LTE:
            ...
            if (ret == XPLR_BOARD_ERROR_OK) {
                ...
                BOARD_CHECK(ret == XPLR_BOARD_ERROR_OK, "LTE reset failed", ret);
                err = gpio_set_level(BOARD_IO_UART_LTE_DTR, 0); /* DTR low for hw flow control*/
                if (err != ESP_OK) {
                    ret = XPLR_BOARD_ERROR;
                    break;
                } else {
                ...
                BOARD_CHECK(ret == XPLR_BOARD_ERROR_OK, "LTE DTR config failed", ret);
            }
            break;
