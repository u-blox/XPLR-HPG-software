...
void app_main(void)
{
    ...
    static bool wifiConnectedInitial = true;
    ...
    while (1) {
            ...
            case XPLR_MQTTWIFI_STATE_CONNECTED:
                ...
                if (gnssState == XPLR_GNSS_STATE_DEVICE_READY) {
                    ...
                    if (espRet != ESP_OK) {
                        ...
                        XPLR_CI_CONSOLE(306, "ERROR");
                        appHaltExecution();
                    } else {
                        XPLR_CI_CONSOLE(306, "OK");
                    }
                } else {
                    if (MICROTOSEC(esp_timer_get_time() - gnssLastAction) >= APP_INACTIVITY_TIMEOUT) {
                        appTerminate();
                    ...
                    if (gnssState == XPLR_GNSS_STATE_DEVICE_READY) {
                        ...
                        if (strcmp(mqttMessage.topic, topicArray[0]) == 0) {
                            ...
                            if (espRet != ESP_OK) {
                                ...
                                XPLR_CI_CONSOLE(308, "ERROR");
                                appHaltExecution();
                            } else {
                                XPLR_CI_CONSOLE(308, "OK");
                            }
                        }
        ...
        */
        ...
        if (deviceOffRequested) {
            ...
            xplrMqttWifiHardDisconnect(&mqttClient);
            xplrGnssStopDevice(gnssDvcPrfId);
            do {
...
static void appInitWiFi(void)
{
    ...
    if (espRet != ESP_OK) {
        ...
        XPLR_CI_CONSOLE(304, "ERROR");
        appHaltExecution();
    } else {
        XPLR_CI_CONSOLE(304, "OK");
    }
}
...
static void appInitGnssDevice(void)
{
    ...
    if (espRet != ESP_OK) {
        ...
        XPLR_CI_CONSOLE(301, "ERROR");
        appHaltExecution();
    } else {
        XPLR_CI_CONSOLE(301, "OK");
    }
...
static void appTerminate(void)
{
    ...
    xplrMqttWifiUnsubscribeFromTopicArray(&mqttClient, topicArray, ELEMENTCNT(topicArray));
    xplrMqttWifiHardDisconnect(&mqttClient);
    xplrLbandStopDevice(lbandDvcPrfId);
